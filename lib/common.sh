...
backup_files() {
    local _f _backup_dir _f2

    _backup_dir="$TOP/work/hosts/`hostname`"
    for _f in "$@"; do
	_f2=${_backup_dir}/SAVED$_f
	
	echo backup $_f to $_f2
	if [ ! -d `dirname $_f2` ]; then
	    mkdir -p `dirname $_f2`
	fi
	
	run_cmd cp $_f ${_backup_dir}/SAVED$_f
	touch cp ${_backup_dir}/SAVED$_f
    done
}
...
save_file_list() {
    ...
    _file_list="$*"
    save_output $TOP/work/hosts/`hostname`/file.lst echo "$_file_list"
}

restore_files() {
    local _f _backup_dir

    _backup_dir="$TOP/work/hosts/`hostname`/SAVED"
    for _f in `(cd $_backup_dir && find . -type f)`; do
	run_cmd cp $_backup_dir/$_f `echo $_f |sed -e 's/^\.//g'`
    done
}
